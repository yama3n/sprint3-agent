...
Foundation段階（Slice 0-7）ではAGENT-01/AGENT-02共通のスケルトン値を置く。
build-loopのエージェントスライスで、AGENT-01/AGENT-02それぞれの実際のミッション・
ツール・完了条件（agent-plan.md 3章・4章）に基づいて具体化する。
"""
...
MAX_TURNS = 15  # 最大ターン数（agent-plan.md の最大ツール呼び出し数15回に対応、SDKに渡す）

# --- タイムアウトの2層構造（必ず 内側 < 外側 を守る） ---
# 内側 = エージェント自身の停止条件。発火したらトレースに記録して整然と終了する。
# 外側 = ジョブ層（jobs.py）のフェイルセーフ。内側がハング等で発火できないときの最後の砦。
INNER_TIMEOUT_S = 120  # 内側: エージェント実行全体の上限（agent-plan.md AGENT-02のタイムアウト値）
