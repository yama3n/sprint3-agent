...
>
> **画面レイアウト方針**: 全画面（ログイン画面を除く）で「左サイドバー + 上部ヘッダー（パンくず）+ メインコンテンツ」の3分割レイアウトに統一する。サイドバーには主要ナビゲーション（引合一覧・新規アップロード・確認中／確定済みフィルタ）とユーザー情報・ログアウトを配置し、常に「現在地」「次に取るべき操作」が分かる状態を保つ。
...
| 要素 | 説明 | 位置 | 対応機能(②) |
|------|------|------|-------------|
| サイドバー | 引合一覧／＋新規アップロード／確認中フィルタ／確定済みフィルタ（各件数バッジ付き）、下部にユーザー情報・ログアウト | 画面左（全画面共通） | FUNC-01, FUNC-07 |
| パンくず | 「引合一覧」を表示 | ヘッダー | - |
