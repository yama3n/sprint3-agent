...
- **02-requirement.md／03-spec.mdとの矛盾がないか**: 固定項目数・区分・「エンジ会社」表記は02-requirement.mdに合わせて統一した（詳細は次節）。一方で、ユーザー向けステータス表現については重要な不整合が残っている。詳細は「7. 未確定事項」の1点目を参照。
...
1. **【重要・要判断】ユーザー向けステータス表現の統一方針が、02-requirement.md FUNC-07および現行03-spec.md／mockup.htmlと異なる。**
   - 今回の指示は「不明／アラート／要確認をユーザー向けの独立した主要ステータスとして分けず、確認が必要な項目は画面上すべて『要確認』とする」というもので、本ドキュメントの完了条件・出力形式にはこの方針（ユーザー向け: 確認不要／要確認の2値、内部理由としてmissing/conflict/ambiguous/multiple_candidates/parse_errorを保持）を反映した。
   - 一方、02-requirement.md FUNC-07は「✅ 確認不要 / ？ 不明 / ⚠️ 要確認」の**3状態**をユーザー向け主表示とし、「不明」は要確認とは別の独立したラベルとして定義している。さらに現行の03-spec.md（4章）とmockup.htmlは、直近の反復で「確認不要／不明／要確認／アラート」の**4状態表示**（アラートは矛盾・曖昧を要確認よりさらに優先度の高い表示として独立させたもの）まで実装済みである。
   - 今回はagent-plan.mdの更新のみが依頼範囲だったため、02-requirement.md・03-spec.md・mockup.htmlは変更していない。UI全体をこの新方針（要確認への一本化）に統一するのであれば、02-requirement.md FUNC-07の記述見直しと、03-spec.md／mockup.htmlのステータス表示（バッジ・件数集計・Inspector内訳表示）の作り直しが必要になる。どちらの方針を正とするか、ご判断をお願いしたい。
