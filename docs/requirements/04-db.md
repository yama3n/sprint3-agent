...
| status | varchar(20) | NOT NULL, CHECK (`running`, `succeeded`, `failed`, `stopped`), DEFAULT `running` | 実行状態。`stopped`は強制停止（agent-plan.md「強制停止・停止条件」） |
...
| error_message | text | NULL可 | 失敗・強制停止時のエラー内容 |
